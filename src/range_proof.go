...
/*
We need to ensure y is invertible.
*/
func ChallengeReduce(y *big.Int) *big.Int {
	for i := range Factors {
		r := new(big.Int).Mod(y, Factors[i])
...
func RPProve(v *big.Int) RangeProof {
	...
	cy = ChallengeReduce(cy)

	rpresult.Cy = cy

//	chal2s256 := sha256.Sum256([]byte(S.X.String() + S.Y.String()))
	cz := new(big.Int).Exp(cy, big.NewInt(int64(len(aL))), EC.N) // z = y^n
...
func RPVerify(rp RangeProof) bool {
	...
	cz := new(big.Int).Exp(cy, big.NewInt(int64(len(EC.BPG))), EC.N) // z = y^n
	if cz.Cmp(rp.Cz) != 0 {
